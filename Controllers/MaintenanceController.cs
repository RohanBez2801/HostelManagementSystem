    ...
    public class MaintenanceController : ControllerBase
    {
        ...
        public IActionResult GetAll()
        {
            ...
            using (OleDbConnection conn = new OleDbConnection(connString))
            {
                try
                {
                    // INNER JOIN ensures we get the human-readable RoomNumber from tbl_Rooms
                    string sql = @"SELECT m.MaintenanceID, m.IssueDescription, m.Priority, m.ReportedDate, m.[Status], r.RoomNumber 
                                 ...
                                 INNER JOIN tbl_Rooms r ON m.RoomID = r.ID 
                    ...
                    using (var reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            list.Add(new
                            {
                                ...
                                ReportedDate = Convert.ToDateTime(reader["ReportedDate"])
                            });
                        }
                    }
                    return Ok(list);
