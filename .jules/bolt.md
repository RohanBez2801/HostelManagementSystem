...
## 2026-01-18 - ADO.NET GetOrdinal Caching
**Learning:** Using string-based indexers (e.g., `reader["Column"]`) inside a `DataReader` loop performs a name-to-ordinal lookup for every row, resulting in O(N*M) lookups where N is rows and M is columns.
**Action:** Always cache column ordinals using `reader.GetOrdinal("Column")` before iterating through `DataReader` results to reduce lookups to O(M), significantly improving performance for large datasets.
